...
def _get_mqtt_cache_for(ret):
    ...
    # Get MQTT cache instance from context if present
    _mqtt_cache = __context__.get("my_mqtt_cache", None)

    # Create new instance if no existing found
    if _mqtt_cache == None:
        options = _get_mqtt_options(ret)
        log.info("Creating MQTT cache instance with Redis options: {:}".format(options))

        _mqtt_cache = MqttCache().setup(redis=options)
        __context__["my_mqtt_cache"] = _mqtt_cache
    elif log.isEnabledFor(logging.DEBUG):
        log.debug("Re-using MQTT cache instance found in context")
